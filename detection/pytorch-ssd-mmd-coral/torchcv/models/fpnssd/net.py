...
class FPNSSD512(nn.Module):
    ...
    box_sizes = (35.84, 76.8, 153.6, 230.4, 307.2, 384.0, 460.8,
                 537.6)  # default bounding box sizes for each feature map.
    ...
    def __init__(self, num_classes):
        ...
        for i in range(len(num_anchors)):
            self.loc_layers += [
                nn.Conv2d(in_channels, num_anchors[i] * 4, kernel_size=3, padding=1)
            ]
            self.cls_layers += [
                nn.Conv2d(in_channels, num_anchors[i] * num_classes,
