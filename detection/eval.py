#####################################################################################
# This file is part of VISDA-18 challenge code for the detection track.
# It calculates the per-category and mean mAP of your predictions 
...
def chunks(seq, k):
    uniq = object()
    it = zip_longest(*[iter(seq)] * k, fillvalue=uniq)
    for elem in it:
        if uniq in elem:
...
def detection_evaluation(ground_truth_file, predictions_file):
	category_names = ['airplane', 'bicycle', 'bus', 'car', 'horse', 'knife', 'motorcycle', 'person', 'plant', 'skateboard', 'train', 'truck']

	with open(ground_truth_file, 'rb') as f:
		gt_boxes, gt_labels = pickle.load(f)
	f.close()

	with open(predictions_file) as f:
	    preds = [
	        [(subseq[:-1], subseq[-1])
	        	for subseq in chunks(line.split(), k=6)]
	        		for line in f.read().splitlines()
	    ]
	f.close()

	pred_boxes = [np.array([[int(x[0][1]), int(x[0][0]), int(x[0][3]), int(x[0][2])] for x in pred]) for pred in preds]
	pred_labels = [np.array([int(x[0][4]) for x in pred]) for pred in preds]
	pred_scores = [np.array([float(x[1]) for x in pred]) for pred in preds]


	if (len(pred_labels) == 0 or len(pred_scores) == 0 or len(pred_boxes) == 0):
		print('Error: prediction file is empty.')
		return

	elif len(pred_labels) != len(gt_labels):
		print('Error: prediction file does not contain the same number of elements as ground truth file.')
		return

	ap_map_pred = eval_detection_voc(pred_boxes, pred_labels, pred_scores, gt_boxes, gt_labels, iou_thresh=0.5, use_07_metric=True)
	ap_pred = ap_map_pred['ap']
	map_pred = ap_map_pred['map']

	with open('scores.txt', 'w') as f:

	    f.write('mAP:' + str(map_pred) + '\n')

	    for idx, category in enumerate(category_names):
	    	f.write(category + ':' + str(ap_pred[idx]) + '\n')
...
if __name__ == '__main__':
    if '--io' in sys.argv:
        ...
        ground_truth_fn = 'val_ground_truth.pkl'
        predictions_fn = 'example_predictions.txt'

    detection_evaluation(ground_truth_fn, predictions_fn)
